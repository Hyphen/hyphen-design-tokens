...
const brandColorOptions = [].concat.apply(
  [],
  Object.keys(brandColors).map((colorName) => colorName)
);
...
const textWrapOptions = Object.keys(font["text-wrap"]);
const textDecorationLineOptions = Object.keys(font["text-decoration-line"]);
const textDecorationStyleOptions = Object.keys(font["text-decoration-style"]);
const textTransformOptions = Object.keys(font["text-transform"]);
const whiteSpaceOptions = Object.keys(font["white-space"]);
const wordBreakOptions = Object.keys(font["word-break"]);

const TEXT_WRAP_OPTIONS = "TEXT_WRAP_OPTIONS";
const TEXT_DECORATION_LINE_OPTIONS = "TEXT_DECORATION_LINE_OPTIONS";
const TEXT_DECORATION_STYLE_OPTIONS = "TEXT_DECORATION_STYLE_OPTIONS";
const TEXT_TRANSFORM_OPTIONS = "TEXT_TRANSFORM_OPTIONS";
const WHITE_SPACE_OPTIONS = "WHITE_SPACE_OPTIONS";
const WORD_BREAK_OPTIONS = "WORD_BREAK_OPTIONS";
...
const createColorTokens = (currentFile) => {
  ...
  result = result.concat(
    writeExport(writeArray(brandColorOptions, BRAND_COLORS))
  );
...
const createFontTokens = (currentFile) => {
  let result = currentFile;

  result = result.concat(writeArray(textWrapOptions, TEXT_WRAP_OPTIONS));
  result = result.concat(
    writeArray(textDecorationLineOptions, TEXT_DECORATION_LINE_OPTIONS)
  );
  result = result.concat(
    writeArray(textDecorationStyleOptions, TEXT_DECORATION_STYLE_OPTIONS)
  );
  result = result.concat(
    writeArray(textTransformOptions, TEXT_TRANSFORM_OPTIONS)
  );
  result = result.concat(writeArray(whiteSpaceOptions, WHITE_SPACE_OPTIONS));
  result = result.concat(writeArray(wordBreakOptions, WORD_BREAK_OPTIONS));
...
const createColorTypes = (currentFile) => {
  ...
  result = result.concat(
    writeExport(writeUnionTypeFromArray("BrandColor", BRAND_COLORS))
  );
  result = result.concat(
...
const createFontTypes = (currentFile) => {
  let result = currentFile;
  result = result.concat(
    writeExport(writeUnionTypeFromArray("TextWrap", TEXT_WRAP_OPTIONS))
  );
  result = result.concat(
    writeExport(
      writeUnionTypeFromArray(
        "TextDecorationLine",
        TEXT_DECORATION_LINE_OPTIONS
      )
    )
  );
  result = result.concat(
  ...
  result = result.concat(
    writeExport(
      writeUnionTypeFromArray("TextTransform", TEXT_TRANSFORM_OPTIONS)
    )
  );
  result = result.concat(
    writeExport(writeUnionTypeFromArray("WhiteSpace", WHITE_SPACE_OPTIONS))
  );
  result = result.concat(
    writeExport(writeUnionTypeFromArray("WordBreak", WORD_BREAK_OPTIONS))
  );
...
const writeFile = () => {
  ...
  tokensData = createFontTokens(tokensData);
  ...
  tokensData = createFontTypes(tokensData);
