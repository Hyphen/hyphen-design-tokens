...
const generateUtilityClass = (utility, prop, variation, breakpoint, state) => {
  ...
  if (
    tokenCategory === utility.tokenCategory &&
    tokenType === utility.tokenType
  ) {
    ...
    if (tokenType === 'spacing' || name === 'border-width') {
      utilityClass = `.${utilityClass} { ${generateShorthandProperties(
        utility,
        prop,
        variation,
      )} }`;
    } else if (tokenType === 'border-radius') {
      utilityClass = `.${utilityClass} { ${generateBorderRadiusShorthandProperties(
        utility,
        prop,
        variation,
      )} }`;
...
const processUtilities = (
  ...
  utilities.forEach(utility => {
    utility.variations.forEach(variation => {
      const utilityClass = generateUtilityClass(
        utility,
        prop,
        variation,
        breakpoint,
...
const utilityClass = {
  ...
  formatter: function (dictionary) {
    ...
    const sortedBreakpoints = breakpoints.sort(
      (a, b) => parseInt(a.original.value) - parseInt(b.original.value),
    );
    ...
    dictionary.allProperties.forEach(prop => {
      ...
      output += processUtilities(hoverUtilities, prop, null, {
        state: 'hover',
      });
    ...
    sortedBreakpoints.forEach(breakpoint => {
      ...
      let responsiveAndHoverUtilities = responsiveUtilities.filter(
        utility => utility.hover,
      );
      let responsiveAndFocusUtilities = responsiveUtilities.filter(
